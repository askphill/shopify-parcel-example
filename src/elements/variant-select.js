  ...
  class VariantSelect extends HTMLElement {
    async connectedCallback() {
      ...
      const prefetchVariants = async () =>
        Array.from(this.querySelectorAll("input[type='radio']")).forEach(async (input) => {
          const query = new URLSearchParams(window.location.search);
          query.set('variant', input.value);
          query.set('sections', 'main-product');

          variantCache[input.value] = await fetch(`${window.location.pathname}?${query}`).then((r) => r.json());
        });
      ...
      if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) {
        ...
        new Promise((resolve) => requestIdleCallback(resolve)).then(prefetchVariants);
      }

      this.querySelectorAll("input[type='radio'][name='variant']").forEach((input) => {
        input.addEventListener('change', async () => {
          ...
          const r = variantCache[input.value]
            ...
            : await fetch(`${window.location.pathname}?${query}`).then((r) => r.json());
