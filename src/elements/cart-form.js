    ...
    connectedCallback() {
      this.unsubscribe = subscribe(
        ...
        ({ sections: { ["cart-drawer"]: section } = {} }) => {
          ...
          if (section) {
            ...
            // remove items
            Array.from(
              this.querySelectorAll('[data-target="cart-form.item"][data-key]')
            ).forEach((item) => {
              if (!html.querySelector(`[data-key="${item.dataset.key}"]`))
                item.remove();
            });

            // add or update items
            Array.from(
              html.querySelectorAll('[data-target="cart-form.item"][data-key]')
            ).forEach((item) => {
              if (!this.querySelector(`[data-key="${item.dataset.key}"]`)) {
                ...
                const before = target.querySelector(
                  `[data-target="cart-form.item"][data-index="${
                ...
                this.querySelector(
                  `[data-target="cart-form.item"][data-key="${item.dataset.key}"]`
            ...
            this.querySelectorAll(
              ...
              e.replaceWith(
                html.querySelector(`[data-target="${e.dataset.target}"]`)
              )
            );
          }
        }
      ...
      this.querySelector("form").addEventListener(
        ...
        debounce(async (e) => {
          ...
          e.target
            .closest('[data-target="cart-form.item"]')
            .toggleAttribute("data-loading", true);
